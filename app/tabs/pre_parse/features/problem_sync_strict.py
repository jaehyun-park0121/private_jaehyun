...
class PreParseProblemSyncStrictMixin:
    def _problem_sync_is_busy(self, action_name: str) -> bool:
        if bool(getattr(self, "_pre_parse_snapshot_loading", False)):
            ...
            return True
        if bool(getattr(self, "_pre_parse_is_running", False)):
            QMessageBox.information(self, "실행 중", f"검사 실행이 끝난 뒤 {action_name}을 진행해 주세요.")
            self._set_recent_log(f"{action_name} 보류: 검사 실행 중")
            return True
        return False
    ...
    def _problem_sync_terminal_log(self, message: str) -> None:
        try:
            print(f"[SKIP][pre_parse problem_sync_strict] {str(message).strip()}", flush=True)
        except Exception:
            pass

    def _problem_sync_row_label(self, row: Dict[str, Any]) -> str:
        page_key = str(row.get("page_no", "")).strip() or "-"
        ...
        return f"{page_key} / shape {shape_text} / label {label_id}:{label} / row_key={row_key}"

    def _visible_current_rows(self) -> List[Dict[str, Any]]:
        visible_keys = [str(key).strip() for key in self.center_panel.visible_row_keys() if str(key).strip()]
        if not visible_keys:
            return []
        ...
        for row_key in visible_keys:
            current_row = current_by_key.get(row_key)
            if current_row is None or row_key in seen_keys:
                continue
    ...
    def _rebuild_reset_reason_menu(self, menu: QMenu) -> None:
        ...
        for reason, count in reason_items:
            action = menu.addAction(f"{reason} ({count}건)")
    ...
    def _problem_sync_json_row_for_page(self, book_id: str, page_token: str) -> Optional[dict]:
        json_row = self._snapshot_json_row_for_page(book_id, page_token)
        if isinstance(json_row, dict):
            return json_row
        return self._resolve_json_row(book_id, page_token)
    ...
    def _problem_sync_load_snapshot_payload(
        ...
        del json_row
        return self._load_snapshot_page_json_payload(book_id, page_token)
    ...
    def _problem_sync_prefetch_page_context(
        ...
        *,
        ...
        page_label = f"{book_id}:{page_token}"
        page_key = (book_id, page_token)
        json_row = self._problem_sync_json_row_for_page(book_id, page_token)
        if json_row is None:
            return {
                "status": "missing_json",
                "page_key": page_key,
    ...
    def _problem_latest_payloads_for_pages(
        ...
        *,
        ...
        if worker_count <= 1:
            for index, (book_id, page_token) in enumerate(page_keys, start=1):
                if prefetch_desc:
                    self._set_progress(prefetch_desc, index, total_pages)
                QApplication.processEvents()
                result = self._problem_sync_prefetch_page_context(book_id, page_token, loader=s3_loader)
                page_label = str(result.get("page_label", "")).strip() or f"{book_id}:{page_token}"
                status = str(result.get("status", "")).strip()
                if status == "ok":
                    page_contexts[(book_id, page_token)] = {
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=worker_count) as executor:
                future_map = {
                    ...
                    for book_id, page_token in page_keys
                }
                completed = 0
                for future in concurrent.futures.as_completed(future_map):
                    ...
                    book_id, page_token = future_map[future]
                    page_label = f"{book_id}:{page_token}"
                    try:
        ...
        for book_id, page_token in page_keys:
            page_context = page_contexts.get((book_id, page_token))
            if not page_context:
                continue
            ...
            if not isinstance(json_row, dict) or not isinstance(latest_payload, dict):
                ...
                page_contexts.pop((book_id, page_token), None)
                continue

            snapshot_payload = self._problem_sync_load_snapshot_payload(book_id, page_token, json_row)
    ...
    def _show_problem_snapshot_mismatch_warning(self, title: str, mismatch_pages: List[str]) -> None:
        ...
        QMessageBox.warning(
            ...
            title,
            (
                f"스냅샷과 다른 JSON이 {len(mismatch_pages)}건 감지되어 작업을 진행하지 않습니다.\n"
    ...
    def _sync_problem_fields_to_json(
        ...
        *,
        ...
        for row in rows:
            ...
            book_id, page_token = self._split_issue_page(page_key)
            if not book_id or not page_token:
                continue
            grouped.setdefault((book_id, page_token), []).append(row)
        ...
        try:
            ...
            return 0, [f"{book_id}:{page_token}" for book_id, page_token in grouped.keys()], []
        ...
        for (book_id, page_token), target_rows in grouped.items():
            page_context = page_contexts.get((book_id, page_token))
            if not page_context:
                continue
    ...
    def _mark_problem_for_visible_rows(self) -> None:
        ...
        self._set_recent_log(
            f"확인 필요 체크 완료: 범위=현재 필터 결과 / 수정 페이지 {updated_pages}건 / 대상 행 {len(target_rows)}건"
        )
        if failed_pages:
            preview = ", ".join(failed_pages[:3])
            suffix = "" if len(failed_pages) <= 3 else " ..."
