...
class S3Loader:
    ...
    def _build_client(self):
        try:
            ...
            raise RuntimeError("boto3가 설치되어 있지 않습니다.") from exc
        ...
        if not access_key or not secret_key:
            raise ValueError("AWS Access Key와 Secret Key가 필요합니다.")
    ...
    def parse_s3_path(s3_path: str) -> Tuple[str, str]:
        value = str(s3_path or "").strip()
        if not value.startswith("s3://"):
            raise ValueError("S3 경로는 s3:// 로 시작해야 합니다.")
        body = value[5:]
        if not body:
            raise ValueError("S3 버킷명이 비어 있습니다.")
        if "/" in body:
